...
fun main(args: Array<String>) {
        ...
        ArgParser(
            ...
            helpFormatter =
                DefaultHelpFormatter(
                    epilogue =
                        "Discord bot 1token can also be set with the DISCORD_THREAD_SCHEDULER_TOKEN environment variable"
                ),
        )
    val args =
        try {
    ...
    if (args.token != null && args.tokenFile != null) {
        System.err.println("dts: please do not set both TOKEN and TOKEN_FILE arguments together")
        exitProcess(3)
    }
...
fun readToken(tokenFile: String?): String? {
    ...
    val token =
        try {
            ...
            System.err.println("dts: TOKEN_FILE was not found ($path)")
            exitProcess(5)
        }
    if (token.isBlank()) {
        System.err.println("dts: TOKEN_FILE was found but is empty ($path)")
        exitProcess(6)
    }
    return token
}
...
fun readConfig(configFile: String): Config {
    ...
    return try {
        ...
        System.err.println("dts: CONFIG_FILE was not found ($path)")
        exitProcess(7)
    } catch (e: SerializationException) {
        System.err.println("dts: CONFIG_FILE was found but could not be deserialized ($path)")
